...
\title{NeuroAdaptive Interface (NAI): Real-Time P300 Detection using EEGNet and Cross-Subject Evaluation}
...
EEG signals vary wildly between individuals. Skull thickness, cortical geometry, electrode contact, and background alpha rhythms---all of these shift the statistics of the recorded signal in ways that confound machine learning models. A classifier trained on Alice may perform at chance on Bob, even when both show clear P300 responses to the same oddball task \cite{sutton1965p300,krusienski2006comparison}. This inter-subject gap is well documented, yet many published systems sidestep it by mixing subjects during training and testing, or by reporting only within-subject accuracy.
...
For LOSO, we ran 20 folds. In each fold, 19 subjects contributed training data (roughly 3768 epochs pooled) and the remaining subject served as the test set (around 200 epochs). Standardization zero mean, unit variance was computed on training data only and then applied to the held-out subject. We ran everything on a laptop (13th Gen Intel i5-13420H, 16~GB RAM); no GPU acceleration was used. Random seed was fixed at 42 for reproducibility. Full parameter tables appear in Appendix~A.
...
\begin{table}[H]
\centering
\caption{Classification comparison (LOSO cross-validation, 20 folds). Accuracy is reported for completeness but is misleading due to class imbalance (see text).}\label{tab:auc}
\begin{tabular}{lccc}
\toprule
